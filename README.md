...
- Подзадачи определяются рекурсивно через фильтр Planfix «Непосредственная
...
- каждый комментарий — блок `- {дата-время} · [{commentId}](https://{host}/task/{id}/?comment={cid})`
  со ссылкой на комментарий в Planfix.
...
Пример — выгрузить задачи, срок которых раньше 01.08.2026:

```
export_tasks(output_dir="backup", end_date_before="2026-08-01", include_comments=true)
```
